...
router.get('/:id?',async (req, res) => {
    try {
        ...
        if (id) {
            const [results] = await pool.query(`SELECT * FROM ${TABLE.PAGES_TABLE} WHERE status !=0 and id = ? `, [id]);
            if (results.length > 0) {
        ...
        const [results] = await pool.query(`SELECT * FROM ${TABLE.PAGES_TABLE} WHERE status = 1 ORDER BY id DESC `);
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        if (title) {
            const [existingTitle] = await pool.query(`SELECT * FROM ${TABLE.PAGES_TABLE} WHERE title = ? AND id != ?`, [title, id]);
            if (existingTitle.length > 0) {
                return res.status(400).json({ error: 'Record already exists', status: false });
            }
        }

        let updateQuery = `UPDATE ${TABLE.PAGES_TABLE} SET `;
        let queryParams = [];
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            await pool.query(`SELECT * FROM ${TABLE.PAGES_TABLE} WHERE status = 1 and id = ?`, [deletedId]);
        }));

        const query = `UPDATE ${TABLE.PAGES_TABLE} SET status = 0 WHERE id IN (?)`;
