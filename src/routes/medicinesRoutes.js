...
const { uploadMedicine } = require('../utils/multerConfig');
const {multerErrorHandler} = require('../utils/common');


// Add
router.post('/', uploadMedicine,multerErrorHandler, authMiddleware,async (req, res) => {
    try {
        ...
        const [existingTitle] = await pool.query(`SELECT * FROM ${TABLE.MEDICINES_TABLE} WHERE status = 1 and title = ?`, [title]);
    ...
    // ];
        ...
        await pool.query(`INSERT INTO ${TABLE.MEDICINES_TABLE} (title,image_url) VALUES (?,?)`, [title,image_url]);
...
router.get('/:id?', async (req, res) => {
    try {
        ...
        if (id) {
            const [results] = await pool.query(`SELECT * FROM ${TABLE.MEDICINES_TABLE} WHERE status = 1 and id = ?`, [id]);
            if (results.length > 0) {
        ...
        const [results] = await pool.query(`SELECT * FROM ${TABLE.MEDICINES_TABLE} WHERE status = 1 ORDER BY ID DESC`);
...
// Update
router.put('/:id', uploadMedicine, multerErrorHandler, authMiddleware, async (req, res) => {
    try {
        ...
        const [existingMedicine] = await pool.query(
            `SELECT * FROM ${TABLE.MEDICINES_TABLE} WHERE status = 1 and id = ?`, 
            [id]
        );
        ...
        const [duplicateTitle] = await pool.query(
            `SELECT * FROM ${TABLE.MEDICINES_TABLE} WHERE status = 1 and title = ? AND id != ?`, 
            [title, id]
        );
        ...
        const baseUrl = req.protocol + '://' + req.get('host') + '/uploads/medicines/';
        const image_urls = req.files.map(file => baseUrl + file.filename);
        const image_url = image_urls.length > 0 ? image_urls[0] : existingMedicine[0].image_url; // Keep existing image if none provided

        // Update the medicine
        await pool.query(
            `UPDATE ${TABLE.MEDICINES_TABLE} 
             SET title = ?, 
                 image_url = ?, 
                 updated_at = NOW() 
             WHERE id = ?`,
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            await pool.query(`SELECT * FROM ${TABLE.MEDICINES_TABLE} WHERE status = 1 and id = ?`, [deletedId]);
        }));

        const query = `UPDATE ${TABLE.MEDICINES_TABLE} SET status = 0, deleted_at = NOW() WHERE id IN (?)`;
